...
@Composable
fun UserInputDetailsTemplateScreen(title: String, indicatorPosition: Int, description: String, buttonText: String = "NEXT", buttonClick: () -> Unit = {}, content: @Composable () -> Unit) {
    Box(
        ...
        Column(modifier = Modifier.fillMaxSize().padding(20.dp)) {
            ...
            PageIndicator(modifier = Modifier.padding(top = 30.dp), amount = 4, currentPosition = indicatorPosition)
            Text(modifier = Modifier.padding(top = 30.dp), text = description, fontSize = 20.sp, color = Color.White)
            content()
            Spacer(modifier = Modifier.weight(1f))
            TextBaseButton(modifier = Modifier.fillMaxWidth().height(LocalConfiguration.current.screenHeightDp.dp.div(20)), text = buttonText, textColor = PrimaryColorNavy, backgroundColor = Color.White, onClick = { buttonClick() }) {
            }
        }
    }
}
